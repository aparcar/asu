...
class PackageResolver:
    ...
    def resolve(
        self, prepare_request: PrepareRequest
        ...
        Args:
            prepare_request: The prepare request to resolve packages for
        ...
        original_packages = prepare_request.packages.copy()

        # Apply package changes (existing logic from package_changes.py)
        apply_package_changes(prepare_request)

        # Track what changed
        self._track_changes(original_packages, prepare_request.packages, prepare_request)

        return prepare_request.packages, self.changes
    ...
    def _track_changes(
        ...
        for removed_pkg in list(removed):
            ...
            migration = self._find_migration(
                removed_pkg, prepare_request.version, prepare_request
            )
            if migration and migration in added:
                self.changes.append(
                    PackageChange(
                        ...
                        reason=f"Package renamed in {prepare_request.version}",
                        automatic=True,
                    )
                )
        ...
        # Remaining additions
        for pkg in added:
            reason = self._get_addition_reason(pkg, prepare_request)
            self.changes.append(
                PackageChange(
    ...
    def _find_migration(
        self, package: str, version: str, prepare_request: PrepareRequest
    ) -> Optional[str]:
        ...
        from package_changes import language_packs
    ...
    def _get_addition_reason(self, package: str, prepare_request: PrepareRequest) -> str:
        """Determine why package was added"""
        if package.startswith("kmod-"):
            # Check if it's a hardware-specific module
            if prepare_request.target:
                return f"Required kernel module for {prepare_request.target}"
